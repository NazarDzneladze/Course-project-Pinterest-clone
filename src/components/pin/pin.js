...
export function Pin({ id, img, avatar, nickname, remove, complain, modal }) {
  ...
  this.pinContentBlock.classList.add(
    "pinterest-board__content",
    "pinterest-board__content_hover"
  );
  ...
  this.addOnBoardBtn = new Button({
    label: "Добавить на доску",
    onClick: this.id,
  });
  this.addOnBoardBtn.rootElement.classList.add("btn", "btn-primary");
  this.addOnBoardBtn.rootElement.setAttribute("data-bs-toggle", "modal");
  this.addOnBoardBtn.rootElement.setAttribute(
    "data-bs-target",
    "#exampleModal"
  );
  // Кнопка "Добавить пин на доску"
  ...
  this.hideFromBoardBtn = new Button({
    label: "Скрить пин со страницы",
  ...
  this.pinDropdownContent.append(
    this.addOnBoardBtn.rootElement,
    this.hideFromBoardBtn.rootElement,
    this.complainBtn.rootElement
  );
